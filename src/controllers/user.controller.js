...
const renewAccessToken = asyncHandler(async(req, res)=>{
    ...
    const user = jwt.verify(token, process.env.REFRESHTOKEN_SECRET);
    
    console.log("dekfjwef")
    if(!user) {
    ...
    const {accessToken, refreshToken} = tokens;
    return res.status(200).cookie("accessToken", accessToken, options).cookie("refreshToken", refreshToken, options).json(new ApiResponse(200, {accessToken, refreshToken}, "Successfully renewed tokens."))
...
export {registerUser, loginUser, logoutUser, resetPassword, renewAccessToken}